...
def generate_model_name(folder,
                  type_sig = 'ECG',
                  leads = 16,
                  timepoints = 150,
                  feat = 17,
    ...
    '''
    generate model name based on arguments provided. This function can be used to generate model names for both initial training
    and transfer learning

    :param folder:          (str) path name of model
    :param type_sig:        (str) 'ECG' (default) or 'EGM'
    :param leads:           (int) number of leads (default: 16)
    :param timepoints:      (int) length of signals (default: 150)
    :param feat:        (int) number of phi classes (default: 17)

    :return:                output_name
    '''

    # if feat is greater than 2, then it is for PHI classification
    if feat>2:
        flag = str(feat) + 'PHI_' + str(timepoints)
    # If 2 or less, it is for Z_RHO regression
    else:
        flag = 'Regression_' + str(timepoints)

    output_name = folder + type_sig + '_' + str(leads) + 'leads_' + flag + 'timepoints_LSTMS128_64_attention_' + str(n_batch) +'batch_' + str(n_epochs) + 'epochs_earlystop'
...
def read_training(filename, leads=16, flag_weights=True, flag_norm=True):
    ...
    if all(elem in field_dict for elem in ['data_train','label_train']):
        ...
        return data_train, label_train, weights

def read_testing(filename, leads=16, flag_norm=True):
...
def plot_training_validation_curves(training_file, metrics):
    ...
    loss_train = data['loss'].values
    loss_test = data['val_loss'].values

    acc_train = data[metrics.lower()].values
    acc_test = data['val_' + metrics.lower()].values
    ...
    print('Saving ... %s' % (training_file[:-4]+'.png'))
    fig.savefig(training_file[:-4]+'.png')
...
def transfer_learning_testing(data_test,
    ...
    for a in start_vt:
        ...
        # Computing accuracy only when sum of label test is >1
        if sum(label_test[flag.upper()])>=1:
            if 'phi' in flag.lower():
                testing_score = accuracy_score(np.argmax(predicted, axis=1),np.argmax(label_test[flag.upper()], axis=1)) * 100
                print('Accuracy score: %.3f\n' % (testing_score))
            else:
                testing_score = mean_absolute_error(predicted, label_test[flag.upper()], multioutput='raw_values')
                print('Neg mean absolute error: [%.3f,%.3f]\n' % (
                testing_score[0], testing_score[1]))


        print('\n\nPrinting out predicted values in %s\n\n' % (outfile + str(a)+'start.csv'))
        np.savetxt(outfile + str(a) +'start.csv', predicted)
